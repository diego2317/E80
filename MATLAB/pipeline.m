...
clear
clf
source = 'firstPierRun.mat'; % Raw Data filename
Fs = 10.1;
dt = 1/Fs;
[acc, gyro, mag] = calibrateData(source);
fusedAcc = FuseData.fuseABoring(acc);
...
% figure(1);
% subplot(4,1,1);
% plot(t, roll   *180/pi), ylabel('roll (°)');
% subplot(4,1,2);
% plot(t, pitch  *180/pi), ylabel('pitch (°)');
% subplot(4,1,3);
% plot(t, yaw    *180/pi), ylabel('yaw (°)');
% subplot(4,1,4);
% plot(t, depth),       ylabel('z (m)'), xlabel('time (s)');

%% Yaw drift complimentary filter
alpha = 0.98;      % gyro trust factor
...
subplot(2,1,2);
plot(t, dxdt, 'b-', 'DisplayName', 'Estimate from Depth'); hold on
