...
clear
clf
source = 'curios.mat'; % Raw Data filename
Fs = 10.1;
dt = 1/Fs;
...
% figure('Name', 'Gyro-Mag Complementary Filter');
% plot(t, yaw_cf*180/pi, 'DisplayName', 'Gyro+Mag Complementary Filter'); hold on
% plot(t, yaw*180/pi, 'DisplayName', 'Magnetometer Only');
% ylabel('yaw (°)'), xlabel('time (s)'), grid on;
% title('Yaw Measurement');
% legend('Location', 'Best');
yaw = smoothdata(yaw, 10, 'movmean');
figure('Name', 'Madgwick-Based Orientation');

subplot(3,1,1);

plot(t, roll*180/pi, 'LineWidth',1), ylabel('roll (°)');
title("Madgwick-Based Orientation Estimation");
grid on
set(gca, 'fontsize', 14);
subplot(3,1,2);
plot(t, pitch*180/pi, 'LineWidth',1), ylabel('pitch (°)');
grid on
set(gca, 'fontsize', 14);
subplot(3,1,3);
plot(t, yaw_cf*180/pi, 'LineWidth',1), ylabel('yaw (°)');
xlabel("Time (s)");
grid on
set(gca, 'fontsize', 14);
...
hold on;
plot(t, depth,  'b-', 'DisplayName','Pressure Sensor Measured Depth');
plot(t, z_kf,   'r-','DisplayName','Kalman Filter Estimated Depth');
% plot(t, z_smooth, 'g-', 'DisplayName', 'RTS Smoothed Estimated Depth')
...
% ylim([-0.1 0.1]);
% subplot(2,1,2);
% %plot(t, zkf_errp, 'r-', 'DisplayName','RTS error [m]'); hold on;
% plot(t, zsmth_err, 'g-', 'DisplayName','RTS error [m]');
