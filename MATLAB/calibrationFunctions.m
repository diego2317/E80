classdef calibrationFunctions
    methods(Static)
    function calibratedAccel = accCal(raw, c)
        calibratedAccel = ((c.R * (double(raw).*c.sens - c.bias)')');
    end
    function calibratedGyro = gyroCal(raw, c)
        calibratedGyro = ((c.R * (double(raw).*c.sens - c.bias )')');   % rad/s
    end
    function calibratedMag = magCal(raw, c)
        calibratedMag = ((c.msoft * (double(raw) - c.offset )')');
    end
